...
export function RoleForm({ permissions, initialData, onSuccess }: {
    ...
    const categories = [
        ...
        { label: "Access Control", keywords: ["role", "permission", "backup"], icon: Shield },
        { label: "Finance", keywords: ["salary", "payroll"], icon: Wallet }
    ]
    ...
    const canSubmit = !!watchedName?.trim() && (watchedPermissionIds?.length ?? 0) > 0 && form.formState.isValid && !loading
    ...
    async function handleConfirmedSubmit() {
        ...
        try {
            if (initialData) {
                await api.put(`/roles/${initialData.id}`, pendingValues)
                toast.success("Role updated successfully")
            } else {
                await api.post("/roles", pendingValues)
    ...
    return (
        <>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="flex justify-center">
                        <Button
                            type="submit"
                            className="w-full py-6 text-lg font-bold rounded-2xl bg-cyan-500 text-white border border-cyan-500 hover:bg-cyan-600 shadow-lg shadow-cyan-500/25 disabled:bg-cyan-400 disabled:text-white disabled:border-cyan-400 disabled:shadow-none disabled:opacity-70"
                            disabled={!canSubmit}
                        >
            ...
            <AlertModal
                ...
                loading={loading}
                title={initialData ? "Save Changes" : "Create Role"}
                description={
                    initialData
                        ? `Are you sure you want to save changes to "${pendingValues?.name}"?`
                        : `Are you sure you want to create the role "${pendingValues?.name}"?`
                }
                variant="primary"
                confirmText={initialData ? "Save Changes" : "Create Role"}
                cancelText="Cancel"
            />
        </>
    )
