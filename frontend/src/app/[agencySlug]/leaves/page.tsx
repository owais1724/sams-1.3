...
export default function LeavesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.employeeId) {
      toast.error('Only enlisted personnel can initiate leave protocols')
      setSubmitting(false)
      return
    }
    ...
    try {
      ...
      toast.success('Leave protocol submitted to grid')
  ...
  const handleApproval = async (leaveId: string, status: string, rejectionReason?: string) => {
    ...
    try {
      ...
      toast.success(`Protocol ${statusLabel}`)
      fetchLeaveRequests()
    } catch (error) {
  ...
  return (
    <div className="space-y-12 pb-20">
      <PageHeader
        ...
        subtitle="High-level institutional oversight of personnel availability and emergency deployment bypasses."
        action={
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[500px] border-none rounded-[40px] shadow-2xl p-0 overflow-hidden bg-white">
                <DialogHeader className="p-10 bg-slate-900 text-white relative overflow-hidden">
                  ...
                  <DialogTitle className="text-3xl font-black tracking-tight leading-none z-10 uppercase">Initialize Protocol</DialogTitle>
                  <DialogDescription className="text-slate-400 font-bold text-[10px] mt-3 uppercase tracking-[0.2em] z-10">Verification of structural absence</DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="p-10 space-y-8">
                  <div className="space-y-0">
                    <FormLabelBase label="Absence Protocol Type" required />
                    <Select value={formData.leaveType} onValueChange={(value) => setFormData({ ...formData, leaveType: value })}>
                      ...
                      <SelectContent className="rounded-2xl border-slate-100 p-2">
                        <SelectItem value="SICK" className="py-3 font-bold rounded-xl">Sick / Medical Node</SelectItem>
                        <SelectItem value="CASUAL" className="py-3 font-bold rounded-xl">Personal / Casual</SelectItem>
                        <SelectItem value="ANNUAL" className="py-3 font-bold rounded-xl">Annual Base Cycle</SelectItem>
                        <SelectItem value="EMERGENCY" className="py-3 font-bold rounded-xl">Emergency Deployment Bypass</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-0">
                      <FormLabelBase label="Commencement" required />
                      <Input
                        type="date"
                    ...
                    <div className="space-y-0">
                      <FormLabelBase label="Termination" required />
                      <Input
                        type="date"
                  ...
                  <div className="space-y-0">
                    <FormLabelBase label="Operational Justification" required />
                    <Textarea
                      value={formData.reason}
                      onChange={(e) => setFormData({ ...formData, reason: e.target.value })}
                      placeholder="Provide formal mission-critical justification..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <StatCard title="Active Protocols" value={leaveRequests.filter(r => r.status === 'AGENCY_APPROVED').length} icon={<CheckCircle2 />} color="emerald" />
        <StatCard title="Institutional Log" value={leaveRequests.length} icon={<FileText />} color="blue" />
